...
async def _import_docs(path: str, glob_pattern: str) -> None:
    ...
    try:
        total_chunks = 0
        with console.status("[bold blue]Processing documents..."):
...
async def _search_kb(query: str, top_k: int) -> None:
    """Search the knowledge base using keyword search (BM25)."""
    ensure_data_dirs()
    init_db()
    ...
    try:
        ...
        # Build BM25Index
        docs = [
            {"id": row["id"], "text": row["content"], "doc_title": row["doc_title"], "chunk_index": row["chunk_index"]}
            for row in rows
        ]
        bm25 = BM25Index()
